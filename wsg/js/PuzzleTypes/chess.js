...
Puzzle.Types['ChessBoard'] = {
  tags: ['Chess'],
  'settings': {
    'labels': EnumeratedValue([true, false], true),
    'blank': EnumeratedValue([true, false], true),
  },
  'content': {
    'fen': function(sett) {
      if (sett.value('blank') === true) {
        return '';
      } else {
        return randomSample(chessFens());
      }
    }
  },
  'layout': {
  ...
  makePuzzleBody: function(puzz) {
    so = puzz.settings.sampledObject()
    const {
      labels,
      blank
    } = so

    var rowLabels = range(1, 9, 1).map(x => labels ? x : '')
    var colLabels = allLetters().slice(0, 8).map(x => labels ? x : '')
    var pieces = Puzzle.Types['ChessBoard'].positionsFromFEN(puzz.content.value('fen'))
    var container = gridOfBoxes(8, 8, true, [rowLabels, colLabels], pieces, puzz.layout.object())

    return container;
  },

  positionsFromFEN: function(fenString) {
    const chessPieces = {
      'K': '&#9812',
      'Q': '&#9813',
      'R': '&#9814',
    ...
    // split FEN into rows
    const fenRows = fenString.split(' ')[0].split('/').reverse()
    // parse the FEN rows to fill the position matrix
    var col
    fenRows.forEach(function(row, rowInd) {
      col = 0
      for (var i = 0; i < row.length; i++) {
        ...
        if (/\d/.test(row[i])) {
          ...
          positions[rowInd][col] = chessPieces[row[i]]
          col++
        }
      }
    })
