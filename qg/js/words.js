// longWords = words10k.filter(x=>x.length>=8)
// shortWords = ['cat','rat','ate','eat','see','has','for','ear','ten','ear','tip','pin','the','not','are','red','tan','win','her','him','were','hat','get','now','who','new','was','can','but','our','may','out','use','any','his','did','man','win','add','sit','how','thin','sin','rest','best','six','and','use','has','top','map','she','war','say']
// counts = longWords.map(x=>[x,shortWords.map(y=>x.includes(y)).filter(z=>z==true).length])
// best = counts.sort((a,b)=>b[1]-a[1]).filter(x=>x[1]>=3).slice(0,40);

var bigWordsContainingSmallerWords = [
  "candidate",
  "appeared",
  "warehouse",
  "housewares",
...
var wordsMedium = words10k.filter((x) => x.length < 8 && x.length >= 4);
